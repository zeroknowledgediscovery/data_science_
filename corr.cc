...
int main(int argc,char* argv[])
{
  ...
  options_description desc( "\n### util CORR ### (ishanu chattopadhyay 2018)");
  ...
  map<unsigned int,vector<double> > G;
  for(map<unsigned int,vector<double>>::iterator itr=H.begin();
      itr!=H.end();
      ++itr)
    for(unsigned int i=0;i<itr->second.size();++i)
      G[i].push_back(itr->second[i]);

  map<unsigned int,map<unsigned int,double> > M;
  for(unsigned int i=0;i<G.size();++i)
      for(unsigned int j=0;j<G.size();++j)
	M[i][j]=gsl_stats_correlation(&G[i][0],1,&G[j][0],1,H.size());


  for(map<unsigned int,map<unsigned int,double> >::iterator itr=M.begin();
      itr!=M.end();++itr)
    {
